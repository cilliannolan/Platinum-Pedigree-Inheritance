...
parser.add_argument(
    "-d", "--input-dad", required=True, help="States sites file", type=str
)
parser.add_argument(
    "-m", "--input-mom", required=True, help="States sites file", type=str
)
parser.add_argument(
    "-c",
    "--children",
    required=True,
    help="Semi colon seperated list of children sample ids",
    type=str,
)
parser.add_argument(
    "-mc",
    "--male-children",
    required=True,
    help="Semi colon seperated list of male children sample ids",
    type=str,
)
parser.add_argument(
    "-p",
    "--parents",
    required=True,
    help="Comma seperated list of parent sample ids [dad],[mom] (eg NA12877,NA12878)",
    type=str,
)
parser.add_argument(
    "-o",
    "--output-inht",
    required=True,
    help="out recomb bed file in inht vectors format",
    type=str,
)
parser.add_argument(
    "-g", "--output-gaps", required=True, help="out gaps, prefix, per child", type=str
)
parser.add_argument(
    "-a",
    "--output-analysis",
    required=True,
    help="out snps joined to recombs for simplifying analysis",
    type=str,
)
parser.add_argument(
    "-s", "--output-summary", required=True, help="Summary file", type=str
)
...
def get_recombinations(file_path, parents_dict, sample_id, autosome, children):
    ...
    viterbi_sites["previous_same"] = viterbi_sites.x_seq_opt_state.eq(
        viterbi_sites.x_seq_opt_state.shift()
    )
    viterbi_sites["next_same"] = viterbi_sites.x_seq_opt_state.eq(
        viterbi_sites.x_seq_opt_state.shift(-1)
    )
    viterbi_sites["x_seq_opt_state"] = (
        viterbi_sites["x_seq_opt_state"].str.strip("[]").apply(literal_eval)
    )

    recombs = viterbi_sites.loc[
        ~viterbi_sites["previous_same"] == viterbi_sites["next_same"]
    ]
    recombs["first_or_last"] = False
    recombs.loc[
        recombs.index.isin([recombs.index[0], recombs.index[-1]]), "first_or_last"
    ] = True

    recombs["recombination"] = (
        ~(recombs["previous_same"] & recombs["next_same"]) & ~recombs["first_or_last"]
    )
    recombs["previous_state"] = recombs["x_seq_opt_state"].shift(1)
    recombs["difference_state"] = recombs.apply(
        lambda row: (
            list(set(row["x_seq_opt_state"]) ^ set(row["previous_state"]))
    ...
    recombs["rle"] = (
        recombs["x_seq_opt_state"].ne(recombs["x_seq_opt_state"].shift()).cumsum()
    )
    ...
    haplotypes_pivot = (
        haplotypes[["rle", "x_seq_opt_state"]]
        .explode("x_seq_opt_state")
        .pivot_table(
            index="rle", columns=["x_seq_opt_state"], aggfunc=lambda x: 1, fill_value=0
        )
    )
    ...
    haplotypes = (
        haplotypes[["CHROM", "start", "end", "rle"]]
        .set_index("rle")
        .join(haplotypes_pivot)
    )
    ...
    haplotypes.reset_index(inplace=True)
    haplotypes = (
        haplotypes.set_index(["CHROM", "start", "end"])
        .add_suffix(f"_{parent}")
        .reset_index()
    )

    # Add supporting SNPs, we will use this at the end
    supporting_snps = haplotypes.apply(
        lambda row: filter_values(
            row["start"], row["end"], list(viterbi_sites["start"])
        ),
        axis=1,
    )
...
def output_gaps_analysis(chrom, combined, output_prefix, children, parents_dict):
    ...
    for col in combined[children]:
        ...
        combined[f"{col}_diff"] = combined.apply(
            lambda x: get_hap_difference(x[col], x[f"{col}_shift"]), axis=1
        )

        conditions = [
            (combined[f"{col}_diff"] == "A") | (combined[f"{col}_diff"] == "B"),
            (combined[f"{col}_diff"] == "C") | (combined[f"{col}_diff"] == "D"),
            (combined[f"{col}_diff"] == "AC")
            | (combined[f"{col}_diff"] == "AD")
            | (combined[f"{col}_diff"] == "BC")
            | (combined[f"{col}_diff"] == "BD"),
            (combined[f"{col}_diff"].isnull()),
        ]
    ...
    combined["rle_recomb"] = (
        combined["recombination"].ne(combined["recombination"].shift(-1)).cumsum()
    )
    ...
    for child in children:
        ...
        test = (
            child_df_grouped[child]
            .apply(lambda x: pd.DataFrame(list(x.str.split("").dropna())))
            .iloc[:, 1:-1]
            .groupby("rle")
            .first()
        )
        ...
        test["recomb_parent_dad"] = np.where(
            test["rle_dad"] != test["rle_dad_shift"], "NA12877", ""
        )
        test["recomb_parent_mom"] = np.where(
            test["rle_mom"] != test["rle_mom_shift"], "NA12878", ""
        )
...
def main():
    ...
    parents_dict = {dad_id: "dad", mom_id: "mom"}

    autosome = ["chr" + str(chrom) for chrom in list(range(1, 23))]

    chrom, mom_recombs = get_recombinations(
        args.input_mom, parents_dict, mom_id, autosome, children
    )
    if chrom in autosome:
        dad_chrom, dad_recombs = get_recombinations(
            args.input_dad, parents_dict, dad_id, autosome, children
        )
    elif chrom == "chrX":
        dad_recombs = pd.DataFrame(
            columns=["CHROM", "start", "end"] + [child + "_dad" for child in children]
        )

    if chrom in autosome:
        combined = pd.concat(
            [dad_recombs, mom_recombs], ignore_index=True, sort=True
        ).sort_values(["start"])
        combined = combined.fillna(method="ffill")
        combined = combined.fillna(method="bfill")

        for child in children:
            child_columns = combined[
                combined.columns[pd.Series(combined.columns).str.startswith(child)]
            ]
    ...
    if chrom in autosome:
        combined[
            list(parents_dict.keys())[list(parents_dict.values()).index("dad")]
        ] = "AB"
        combined[
            list(parents_dict.keys())[list(parents_dict.values()).index("mom")]
        ] = "CD"
    elif chrom == "chrX":
    ...
    combined["rle"] = (
        combined["paste_children"].ne(combined["paste_children"].shift()).cumsum()
    )
    first_dict = dict.fromkeys(
        children + ["CHROM", "start", "supporting_snps"] + list(parents_dict.keys()),
        "first",
    )
