...
parser.add_argument("-i", "--input", required=True, help="States sites file", type=str)
parser.add_argument(
    "-c", "--children", required=True, help="Children, comma separated list", type=str
)
parser.add_argument(
    "-m",
    "--male-children",
    required=True,
    help="Male children, comma separated list",
    type=str,
)
parser.add_argument(
    "-l",
...
parser.add_argument(
    "-t", "--transmission-matrix", required=True, help="Out t mat", type=str
)
parser.add_argument(
    "-e", "--emission-matrix", required=True, help="Out e mat", type=str
)
parser.add_argument("-d", "--test-outdir", required=True, help="out", type=str)
parser.add_argument(
    "-p",
    "--punishment",
    required=True,
    help="Comma seperated list of punishment values for snp error, and state change respectively",
    type=str,
)
parser.add_argument("-o", "--output", required=True, help="out", type=str)
parser.add_argument("-f", "--file-prefix", required=True, help="out prefix", type=str)
...
def main():
    ...
    chromosomes = list(
        set.intersection(set(chromosomes), set(list(sites["CHROM"].unique())))
    )
    ...
    with open(args.output, "w") as f:
        for chrom in chromosomes:
            ...
            for parent in parents:
                ...
                filtered_df = sites.query(
                    'CHROM == @chrom & called_parent == @parent & phase in ["A", "B", "C", "D", "hap1", "hap2", "hap3", "hap4"]'
                ).dropna()

                observed_sequence = pd.Series(
                    sites.query(
                        'CHROM == @chrom & called_parent == @parent & phase in ["A", "B", "C", "D", "hap1", "hap2", "hap3", "hap4"]'
                    )[["children_calls", "phase"]].dropna()["children_calls"]
                ).to_list()

                haplotype_list = pd.Series(
                    sites.query(
                        'CHROM == @chrom & called_parent == @parent & phase in ["A", "B", "C", "D", "hap1", "hap2", "hap3", "hap4"]'
                    )[["children_calls", "phase"]].dropna()["phase"]
                ).to_list()
                ...
                filtered_df.to_csv(
                    f"{test_outdir}/{prefix}.{chrom}_{parent}.viterbi_df.txt", sep="\t"
                )
