...
parser.add_argument("-c", "--cohort-calls", required=True, help="Cohort VCF", type=str)
parser.add_argument("-p", "--pedigree", required=True, help="Pedigree file", type=str)
parser.add_argument("-d", "--dad-sample", required=True, help="Dad sample id", type=str)
parser.add_argument("-m", "--mom-sample", required=True, help="Mom sample id", type=str)
parser.add_argument(
    "-s",
    "--subset-children",
    required=False,
    help="Optional. Subset of children from pedigree to consider.",
    type=str,
)
parser.add_argument(
    "-g",
    "--subset-gparents",
    required=False,
    help="Optional. Subset of gparents from pedigree to consider.",
    type=str,
)
parser.add_argument("-f", "--filtered", required=False, help="Variant file", type=str)
...
def filter_variant(
    ...
    if (
        not record.call_for_sample[mom_sample].is_variant
        and not record.call_for_sample[dad_sample].is_variant
    ):
    ...
    if "./." in [
        record.call_for_sample[sample].data["GT"]
        for sample in children + [dad_sample] + [mom_sample] + grandparents
    ]:
    ...
    ## Remove - Hom variant in mom and in chrX
        ...
        called_parent == mom_sample
        and record.CHROM == "chrX"
        and not record.call_for_sample[called_parent].is_het
    ):
...
class parent_info:
    ...
    known_haplotypes = {"A": "B", "B": "A", "C": "D", "D": "C"}
...
def assign_haplotype_snp(record, parent_info, grandparent_dict):
    if len([x for x in list(grandparent_dict.values()) if x is not None]) == 2:
        if (
            record.call_for_sample[grandparent_dict["dad"]].is_variant
            and record.call_for_sample[grandparent_dict["mom"]].is_variant
        ):
...
def main():
    ...
    parents = {"dad": args.dad_sample, "mom": args.mom_sample}

    pedigree = pd.read_csv(args.pedigree, sep="\t").astype(str)

    children_dict = {
        "mom": list(
            pedigree.loc[pedigree["mom"] == parents["mom"]].to_dict()["child"].values()
        ),
        "dad": list(
            pedigree.loc[pedigree["dad"] == parents["dad"]].to_dict()["child"].values()
        ),
    }
    ...
    if args.subset_children:
        ...
        if subset_children.issubset(
            set(children_dict["mom"])
        ) and subset_children.issubset(set(children_dict["dad"])):
            children_dict["mom"] = list(
                subset_children.intersection(set(children_dict["mom"]))
            )
            children_dict["dad"] = list(
                subset_children.intersection(set(children_dict["dad"]))
            )
        else:
            sys.exit(
                f"Subset children specified; {*subset_children,}, not children of mom/dad"
            )
    ...
    mom_parents = {
        "dad": pedigree.loc[pedigree["child"] == str(parents["mom"])].to_dict(
            "records"
        )[0]["dad"],
        "mom": pedigree.loc[pedigree["child"] == str(parents["mom"])].to_dict(
            "records"
        )[0]["mom"],
    }
    dad_parents = {
        "dad": pedigree.loc[pedigree["child"] == str(parents["dad"])].to_dict(
            "records"
        )[0]["dad"],
        "mom": pedigree.loc[pedigree["child"] == str(parents["dad"])].to_dict(
            "records"
        )[0]["mom"],
    }
    ...
    for record in reader:
        ...
        filtered_variant = filter_variant(
            record,
            parents["mom"],
            parents["dad"],
            annotate_filtered_vars,
