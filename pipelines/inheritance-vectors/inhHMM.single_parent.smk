...
try:
    config["ignore_parent"]
    if config["ignore_parent"] == "mom":
        parents_out=[parents[0]]
    else:
        parents_out=[parents[1]]
except KeyError:
    pass
...
rule prepare_snps:
    params:
        ...
        ignore_parent=config["ignore_parent"]
    input:
        script="../../code/inheritance_vectors/prepare_snps.single_parent.py",
    ...
    shell:
        ...
        python {input.script} \\
            ...
            --ignore-parent {params.ignore_parent} \\
            --output {output.bed}
        """
...
rule split_by_chrom:
    ...
    output:
        "output/split/{file_prefix}.{chrom}.{parent}.bed",
    conda:
        "envs/gnugrep.yaml"
    shell:
        """
        c1grep() {{ grep "$@" || test $? = 1; }}
        head -n1 {input.sites} > {output}
        grep -P '^{wildcards.chrom}\t' {input.sites} | c1grep '{wildcards.parent}' >> {output}
        """
...
rule viterbi_to_inht_vectors:
    params:
        ...
        male_children=";".join(male_children)
    input:
        script="../../code/inheritance_vectors/viterbi_to_inht_vectors.single_parent.py",
